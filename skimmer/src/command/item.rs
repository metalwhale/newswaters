...
pub(crate) async fn collect_items(repo: Arc<Mutex<Repository>>, is_job: bool) -> Result<()> {
    let items_num: i32 = env::var("SKIMMER_COLLECT_ITEMS_NUM")
...
pub(crate) async fn collect_item_urls(repo: Arc<Mutex<Repository>>, is_job: bool) -> Result<()> {
    let items_num: i32 = env::var("SKIMMER_COLLECT_ITEM_URLS_NUM")
        .unwrap_or("1000000".to_string())
        .parse()?;
...
pub(crate) async fn summarize_texts(mut repo: Repository, is_job: bool) -> Result<()> {
    let texts_num: usize = env::var("SKIMMER_SUMMARIZE_TEXTS_NUM")
    ...
    loop {
        ...
        if env::var("SKIMMER_SUMMARIZE_ADDITIONAL_TEXTS").is_ok() && items.len() < texts_num {
...
pub(crate) async fn embed_summaries(mut repo: Repository, is_job: bool) -> Result<()> {
    let collection_name = env::var("SEARCH_ENGINE_VECTOR_SUMMARY_COLLECTION_NAME")?;
    let summaries_num: usize = env::var("SKIMMER_EMBED_SUMMARIES_NUM")
        .unwrap_or("1000000".to_string())
        .parse()?;
