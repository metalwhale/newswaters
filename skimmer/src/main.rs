...
async fn main() -> Result<()> {
    ...
    if args.len() >= 2 {
        match args[1].as_str() {
            "items" => save_items(repo).await?,
            "item_urls" => save_item_urls(repo).await?,
            _ => {}
        }
    }
    Ok(())
...
async fn save_items(repo: Arc<Mutex<ItemRepository>>) -> Result<()> {
...
async fn save_item_urls(repo: Arc<Mutex<ItemRepository>>) -> Result<()> {
    let items_num: i32 = env::var("SKIMMER_ITEMS_NUM").unwrap_or("1000000".to_string()).parse()?;
